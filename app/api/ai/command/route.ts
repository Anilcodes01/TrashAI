...
const deleteTask = {
  name: 'deleteTask',
  description: "Deletes an existing task or sub-task from the list. Use this to 'remove', or 'delete' an item.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      itemId: {
        type: Type.STRING,
        description: "The ID of the task or sub-task to delete"
      },
      itemType: {
        type: Type.STRING,
        enum:['task', 'subtask'],
        description: "Specifies whether the item to delete is a 'task' or a 'subtas'",
      },
    },
    required: ['itemId', 'itemType']
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await genAI.models.generateContent({
      ...
      config: {
        tools: [
          { functionDeclarations: [createSubTask, createTask, completeTask, deleteTask] },
        ],
      },
    });
    ...
    if (!args) {
      return NextResponse.json(
        { message: "AI action is missing required arguments." },
        { status: 400 }
      );
    }
    ...
    switch (name) {
      case "createTask": {
        ...
        const newTask = await prisma.task.create({
          data: { content: args.content as string, order: newOrder, todoListId: listId },
        });
      ...
      case "createSubTask": {
        const lastSubTask = await prisma.subTask.findFirst({
          where: { taskId: args.parentTaskId as string },
          orderBy: { order: "desc" },
        });
        const newOrder = (lastSubTask?.order ?? -1) + 1;
        const newSubTask = await prisma.subTask.create({
          data: {
            content: args.subTaskContent as string,
            order: newOrder,
            taskId: args.parentTaskId as string,
          },
        });
        await pusher.trigger(
          `private-list-${listId}`,
          "item-added",
          {
            item: newSubTask,
            itemType: "subtask",
            parentId: args.parentTaskId as string,
          },
          { socket_id: socketId ?? undefined }
        );
        break;
      ...
      case "deleteTask": {
        if (args.itemType === "task") {
          await prisma.task.delete({ where: { id: args.itemId as string } });
        } else {
          const subtask = await prisma.subTask.findUnique({
            where: { id: args.itemId as string },
            select: { taskId: true },
          });
          await prisma.subTask.delete({ where: { id: args.itemId as string } });
          await pusher.trigger(
            `private-list-${listId}`,
            "item-deleted",
            {
      ...
      case "updateTaskContent": {
        if (args.itemType === "task") {
          await prisma.task.update({
            where: { id: args.itemId as string },
            data: { content: args.newContent as string },
          });
        } else {
          await prisma.subTask.update({
            where: { id: args.itemId as string },
      ...
      case "moveTask": {
        if (args.itemType === "task") {
          await prisma.task.update({
            where: { id: args.itemId as string },
            data: { order: args.newOrder as number },
          });
        } else {
          await prisma.subTask.update({
            where: { id: args.itemId as string },
